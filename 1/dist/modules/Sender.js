...
export class Sender extends Message {
    ...
    async handle(payload) {
        ...
        const news = await db.get(group_id)
            .value();
        if (news?.last !== payload.date &&
            !news?.published.includes(payload.date) &&
            new Keywords(keywords).check(payload.text) &&
            (words_blacklist.length ? !new Keywords(words_blacklist).check(payload.text) : true)) { // Проверяем что пост не был опубликован ранее и соответствует ключевым словам
            if ((longpoll && filter && payload.owner_id !== payload.from_id) || // Фильтр на записи "Только от имени группы" для LongPoll API
    ...
    async send() {
        ...
        await Promise.allSettled(webhook_urls.map((url, webhookIndex) => {
            ...
            .then((outputs) => {
            const rejects = outputs.filter(({ status }) => status === "rejected");
            if (rejects.length) {
